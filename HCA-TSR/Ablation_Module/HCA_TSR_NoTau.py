...
# -----------------------------
# HCA-TSR ablation module: tau removed
# -----------------------------
class HCATSRNoTau(nn.Module):
    """HCATSR core ablation module without tau.

    This version removes the temperature coefficient tau and applies gating directly.
    """
    def __init__(
        self,
        input_channel1: int,
        ...
        bias: int = 1,
        gate_eps: float = 1e-6,
        init_tau: float = 1.5,  # Temperature coefficient tau (removed in this ablation)
        debug: bool = False,
    ):
        super().__init__()
        ...
        # Tau is removed here, and gating is applied directly
    ...
    def forward(self, x1, x2):
        ...
        gate = torch.sigmoid(g_cat)  # Do not divide by tau in this ablation
...
# Neck module
# -----------------------------
@MODELS.register_module(name='HCA-TSR-NoTau')
class HCATSRNoTauNeck(BaseModule):
    """HCA-TSR pyramid neck without tau."""
    def __init__(self, in_channels, out_channels: int = 256,
                 norm_type: str = 'BN', init_cfg=None, debug: bool = False):
        super().__init__(init_cfg=init_cfg)
        assert len(in_channels) >= 2, 'HCA-TSR-NoTau requires at least two input feature levels'
        self.num_levels = len(in_channels)
        self.out_channels = out_channels
        self.debug = debug

        self.blocks = nn.ModuleList([
            HCATSRNoTau(in_channels[i], in_channels[i + 1], norm_type=norm_type)
