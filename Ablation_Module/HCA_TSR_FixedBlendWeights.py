...
# -----------------------------
# HCA-TSR ablation module: fixed blend weights
# -----------------------------
class HCATSRFixedBlendWeights(nn.Module):
    """HCATSR core ablation module with fixed blend weights.

    This version fixes the three-branch fusion weights to constants,
    using [0.6, 0.3, 0.1] by default.
    ...
    def __init__(
        ...
        init_tau: float = 1.5,
        debug: bool = False,
        fixed_weights=(0.6, 0.3, 0.1),   # (w_x1, w_x2, w_dt)
    ):
        super().__init__()
        ...
        # --- Upsampling to align with x1 plus lightweight spatial gating ---
        ...
        # --- [FIXED] Constant weights for the three branches ---
        fw = torch.tensor(fixed_weights, dtype=torch.float32)
        fw = fw / fw.sum()  # Normalize to ensure the sum equals 1
        self.register_buffer("fixed_w", fw)  # Keep synchronized with model device and precision

        # Removed learnable weights and epsilon
    ...
    def forward(self, x1, x2):
        ...
        # ---------------- [FIXED] Three-branch constant fusion ----------------
        w = self.fixed_w  # Shape [3], already normalized
...
# -----------------------------
# Neck: HCA-TSR pyramid ablation with fixed blend weights
# -----------------------------
@MODELS.register_module(name='HCA-TSR-FixedBlendWeights')
class HCATSRFixedBlendWeightsNeck(BaseModule):
    """HCA-TSR pyramid neck with fixed blend weights, keeping an FPN-compatible interface.
    ...
    def __init__(
        ...
        assert len(in_channels) >= 2, 'HCA-TSR-FixedBlendWeights requires at least two input feature levels'
        ...
        self.blocks = nn.ModuleList([
            HCATSRFixedBlendWeights(in_channels[i], in_channels[i + 1], norm_type=norm_type,
