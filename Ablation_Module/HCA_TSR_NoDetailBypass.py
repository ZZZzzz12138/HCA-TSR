...
# ----------------------------------------
# HCA-TSR ablation module: DetailBypass removed
# ----------------------------------------
class HCATSRNoDetailBypass(nn.Module):
    """HCATSR core ablation module without DetailBypass.

    This version removes the DetailExtractor branch used in the full HCA-TSR design.
    """
    def __init__(
        self,
        input_channel1: int,
...
# Neck module
# ----------------------------------------
@MODELS.register_module(name='HCA-TSR-NoDetailBypass')
class HCATSRNoDetailBypassNeck(BaseModule):
    """HCA-TSR pyramid neck without DetailBypass."""
    def __init__(self, in_channels, out_channels: int = 256,
                 norm_type: str = 'BN', init_cfg=None, debug: bool = False):
        super().__init__(init_cfg=init_cfg)
        assert len(in_channels) >= 2, 'HCA-TSR-NoDetailBypass requires at least two input feature levels'
        self.num_levels = len(in_channels)
        self.out_channels = out_channels
        self.debug = debug

        self.blocks = nn.ModuleList([
            HCATSRNoDetailBypass(in_channels[i], in_channels[i + 1], norm_type=norm_type)
